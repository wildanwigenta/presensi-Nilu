...
<div class="card col-md-6">
  <div class="card-body ">
    <form method="POST" action="<?= base_url('admin/data_pegawai/store') ?>" enctype="multipart/form-data">
      ...
      <div class="input-style-1">
        <label>No HP</label>
        <input type="text" class="form-control <?= ($validation->hasError('no_hp'))? 'is-invalid' : '' ?>" name="no_hp"
          placeholder="No HP" />
      ...
      <div class="input-style-1">
        ...
        <div class="shift-grid mt-2">
          ...
          <label class="shift-card">
            <input class="shift-checkbox" type="checkbox" name="shift_ids[]" value="<?= $shift['id'] ?>"
              id="shift_<?= $shift['id'] ?>" <?= in_array($shift['id'], old('shift_ids', [])) ? 'checked' : '' ?>>
      ...
      <div class="input-style-1">
        ...
        <div id="face_registration_container" class="border rounded p-3 mb-2"
          style="position: relative; max-width: 340px;">
        ...
        <small class="text-secondary">Webcam akan aktif otomatis, dan sistem akan mendeteksi wajah secara
          realtime.</small>
      </div>
